...
def validate_provider_bars(frame: pl.DataFrame) -> None:
    ...
    if missing_columns:
        raise InvalidMarketDataError(f"Missing required columns: {sorted(missing_columns)}")
    ...
    if (frame["high"] < frame["low"]).any():
        raise InvalidMarketDataError("High values are less than low values")

    if (frame["open"] > frame["high"]).any():
        raise InvalidMarketDataError("Open values are greater than high values")

    if (frame["close"] > frame["high"]).any():
        raise InvalidMarketDataError("Close values are greater than high values")

    if (frame["open"] < frame["low"]).any():
        raise InvalidMarketDataError("Open values are less than low values")

    if (frame["close"] < frame["low"]).any():
        raise InvalidMarketDataError("Close values are less than low values")